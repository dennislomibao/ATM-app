...
int main ()  
{
    ...
    while(1){ /*this while loop will only end when user choose 2*/
        ...
        char a = getchar(); 
        if (a=='1') login(&admin); /*the pre-made admin is passed in*/
        if (a=='2') exit(0);
...
void login(admin_t *admin)
{
    /*loads database file*/
    FILE *pFile;
   int id,x,k; /*id is from the user input, x for isAdmin(), k is customer counter*/
   char password[20];
   /*loads database so app can check login details of customer*/
   ...
   while(fread(&customer[k],1,sizeof(customer_t),pFile)){
    ...
    if(x){ /* x will return 1 if they are admin[0] */
        ...
        adminMain();
        break;
        /* this checks if they match the pre-made login details for customer or an existing customer in database*/
...
{
    /*checks if it matches pre-made admin login details from main()*/
...
{
	int i;
    /*checks customer login details from database */
